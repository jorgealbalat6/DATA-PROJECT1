...
-- Convertir 0.0 a NULL para que COALESCE use el valor del mapa
COALESCE(
    NULLIF(ca.latitud, 0.0),
    m.lat_fix
) AS latitud,
COALESCE(
    NULLIF(ca.longitud, 0.0),
    m.lon_fix
) AS longitud,
...
(
    make_date(ca.ANO, ca.MES, ca.DIA) + make_interval(hours => ca.HORA - 1)
) AS fecha,
ca.valor,
CASE
...
FROM
    calidad_aire ca
    LEFT JOIN mapa_coordenadas m ON ca.codigo_estacion = m.codigo_estacion